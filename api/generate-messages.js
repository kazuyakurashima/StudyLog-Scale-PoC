...
function getPersonalizedFallbackMessages(senderType, subjectName = '', studyData = null, studyHistory = null) {
  ...
  const uniqueSeed = studyData ? 
    `${studyData.subject}_${studyData.date}_${studyData.questionsCorrect}_${studyData.questionsTotal}_${studyData.emotion}_${Date.now()}` : 
    `fallback_${Date.now()}`;
  const randomIndex = Math.abs(uniqueSeed.split('').reduce((a, b) => a + b.charCodeAt(0), 0)) % 100;
  
  console.log('🔄 個別データ反映フォールバックメッセージ生成:', { 
    subjectName, accuracy, correctCount, totalCount, continuationDays, senderType, uniqueSeed, randomIndex 
  });
  ...
  if (senderType === 'parent') {
    ...
    return [
      { message: encouragingMessages[randomIndex % encouragingMessages.length], emoji: "😊", type: "encouraging" },
      { message: specificMessages[(randomIndex + 1) % specificMessages.length], emoji: "🎯", type: "specific_praise" },
      { message: lovingMessages[(randomIndex + 2) % lovingMessages.length], emoji: "💝", type: "loving" }
    ];
  } else {
    // 指導者向けメッセージのバリエーション
    const encouragingMessages = [
    ...
    return [
      { message: encouragingMessages[randomIndex % encouragingMessages.length], emoji: "📈", type: "encouraging" },
      { message: instructionalMessages[(randomIndex + 1) % instructionalMessages.length], emoji: "🎯", type: "instructional" },
      { message: motivationalMessages[(randomIndex + 2) % motivationalMessages.length], emoji: "💪", type: "motivational" }
    ];
  }
}
